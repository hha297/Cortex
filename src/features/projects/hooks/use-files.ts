...
export const useRenameFile = ({ projectId, parentId }: { projectId: Id<'projects'>; parentId?: Id<'files'> }) => {
        return useMutation(api.files.renameFile).withOptimisticUpdate((localStore, args) => {
                ...
                if (existingFiles !== undefined) {
                        const updatedFiles = existingFiles.map((file) => (file._id === args.fileId ? { ...file, name: args.newName } : file));

                        localStore.setQuery(api.files.getFolderContents, { projectId, parentId }, sortFiles(updatedFiles));
                }
        });
};
