...
const validateInternalKey = (key: string) => {
        const internalKey = process.env.CORTEX_CONVEX_INTERNAL_KEY;

        if (!internalKey) {
                throw new Error('CORTEX_CONVEX_INTERNAL_KEY is not configured');
        }
...
export const renameFile = mutation({
        ...
        handler: async (ctx, args) => {
                ...
                const existing = siblings.find((sibling) => sibling.name === args.newName && sibling.type === file.type && sibling._id !== args.fileId);
...
export const updateExportStatus = mutation({
        args: {
                internalKey: v.string(),
                projectId: v.id('projects'),
                status: v.optional(v.union(v.literal('exporting'), v.literal('completed'), v.literal('failed'), v.literal('cancelled'))),
